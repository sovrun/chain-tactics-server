...
export function createUserEnums(filePath: string, namespace: string, enums: any) {
...
export function createAssemblyReference(filePath: string, namespace: string) {
  renderFile(
...
async function main() {
  ...
  Object.entries(tables).forEach(([tableName, { key, schema, name }]) => {
    const filePath = `${outputPath}/${tableName + 'Table'}.cs`
    // const keySchema = key.map((id) => schema[id as keyof typeof schema])
    const keySchema = key.reduce(
      (acc, id) => ({ ...acc, [id]: schema[id as keyof typeof schema] }),
      {} as { [key: string]: SchemaAbiType }
    )

    const valueSchema = Object.entries(schema).reduce(
      (acc, [id, value]) => ({ ...acc, [id]: value['type'] }),
      {} as { [key: string]: SchemaAbiType }
    )

    createTableDefinition(filePath, namespace, mudConfig, tableName, name, keySchema, valueSchema)
  })
